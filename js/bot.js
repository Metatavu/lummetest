(()=>{'use strict';window.metamind=new window.Metamind({apiUrl:'https://api.metamind.fi/v1','story':'lumme-electricity-order'});var botImage='https://robohash.org/'+Math.random().toString(36).substr(2);$(document).on('click','.quick-message-btn',function(e){sendMessage($(this).text())});$('.send-message-btn').click(function(e){sendMessage()});function sendMessage(text){const disabled=$('.send-message-btn').attr('disabled');const message=text||$('.user-text-input').val()||$('.user-date-input').val();if(!message.trim()){return}if(typeof disabled!==typeof undefined&&disabled!==false){return}$('.bot-typing').show();$('.user-text-input').val('');$('.send-message-btn').attr('disabled','disabled');$('#botHintText').text('');$('.quick-responses').empty();$('.discussion-container').append(`<div class="card user-response">
        <div class="card-body">
          <div class="media">
